...
export function StravaImport() {
  ...
  // One shared disclosure for both lists (was two separate accordions) — an
  // audit flagged "Activités Strava" and "Itinéraires Strava" as a redundant
  // extra tap once the auto-organize sync button covers the same ground.
  ...
  return (
    <div className="px-4 pb-4 space-y-1">
      <div className="flex items-center justify-between min-h-11">
        ...
        <button onClick={handleDisconnect} className="text-[10px] text-[var(--color-text-muted)] hover:text-[var(--color-text-primary)] min-h-11 px-1 -mx-1">
          Déconnecter
        </button>
      </div>

      <CollapsibleHeader label="Parcourir Strava" expanded={browseOpen} onToggle={() => setBrowseOpen((v) => !v)} />
